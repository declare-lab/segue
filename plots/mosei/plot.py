...
def main():
    ours_data = pd.read_csv('ours.csv')
    ...
    plot_ours_vs_baseline(
        ours_data['k'][3:], ours_data['MAE'][3:], baseline_data['MAE'][3:],
        np.mean(ours_data['MAE'][:3]), np.mean(baseline_data['MAE'][:3]),
        'MAE',
    )
    plot_ours_vs_baseline(
        ours_data['k'][3:], ours_data['Corr'][3:], baseline_data['Corr'][3:],
        np.mean(ours_data['Corr'][:3]), np.mean(baseline_data['Corr'][:3]),
        'Corr',
    )
    plot_ours_vs_baseline(
        ours_data['k'][3:], ours_data['F1'][3:], baseline_data['F1'][3:],
        np.mean(ours_data['F1'][:3]), np.mean(baseline_data['F1'][:3]),
        'F1',
    )
