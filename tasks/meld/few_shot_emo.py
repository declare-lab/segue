...
def main():
    ...
    result_path = pathlib.Path('output/meld/few_shot_emo.csv')
    result_path.parent.mkdir(parents=True, exist_ok=True)
    with open(result_path, 'w') as file:
            writer = csv.writer(file)
            writer.writerow(['n', 'Seed', 'emo_f1', 'emo_acc'])

    n_values = [1, 2, 4, 8, 16, 32, 64, 128, 256, -1]
    for n, seed in itertools.product(n_values, range(37, 40)):
        ...
        with open(result_path, 'a') as file:
            ...
            writer.writerow([
                n, seed,
                pred['test_emo_f1'], pred['test_emo_acc'],
            ])
